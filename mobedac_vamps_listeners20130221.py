...
class VampsListener():
    ...
    def upload_data_post_with_qual_file(self, seqfile, primfile, metafile, paramfile, qualfile):
        ...
        cursor.execute("insert into vamps_upload_status(user, status, status_message, date) values ('mobedac', 'TRIM_PROCESSING', 'Vamps is processing', '"+today+"');")
    ...
    def upload_data_gast(self, *vpath, **params):
        ...
        dbConn = MySQLdb.connect("localhost", db_user_local, db_passw_local,"vamps_test" )
        cursor = dbConn.cursor()
        sql = "UPDATE vamps_upload_status SET status='GAST_PROCESSING', date='"+today+"' WHERE id in (" + params['run_number'].replace("[","").replace("]","") + ");"
...
def vamps_processor_thread(*args):
    while True:
        ...
        cursor.execute("UPDATE vamps_upload_status SET status='GAST_SUCCESS', date='"+today+"' WHERE status='GAST_PROCESSING';")
        if cursor.rowcount > 0:
            print "Number of rows updated GAST_PROCESSING => GAST_SUCCESS: %d" % cursor.rowcount
        dbConn.commit()
        
        cursor.execute("UPDATE vamps_upload_status SET status='TRIM_SUCCESS', date='"+today+"' WHERE status='TRIM_PROCESSING';")
        if cursor.rowcount > 0:
